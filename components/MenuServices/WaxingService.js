...
export default function WaxingService({prices}) {
  const [isOpen, setIsOpen] = useState(false);
  const {order, setCartItems, cart} = useAuth()
  const service = 'Waxing'
  function handleClick() {
    setIsOpen(!isOpen);
  }
  function getImg(id) {
    return "/../public/waxing/" + id + '.jpg'
  }
    ...
    <section id="waxingservice" className="container px-4 py-10 mx-auto">
      ...
      <div className="grid grid-cols-1 grid-cols-2 md:grid-cols-3 md:grid-cols-4 gap-[30px] ">
        ...
        {prices.map((offer) => (
          ...
          >
            <Image alt="nails-waxingservice" src={getImg(offer.ServiceID)} className="w-[100%]"  width='100' height={100}/>
